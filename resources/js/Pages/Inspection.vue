<template>
  <Show>
    <template #header>
      ...
      <div>
        <h1 class="text-white font-bold ml-1 text-[16px] font-fira tracking-wide">
          {{ trap_data.qr_id }}
        </h1>
      </div>
      <div>
    ...
    <template v-if="step === 1" #last>
      <div
        v-if="step === 1"
        class="bg-t_white-200 w-full flex justify-center items-end py-[6px]"
      >
        <h6
          class="italic text-t_black-800 montserrat text-[12px]"
          v-if="trap_data.last_checked"
        >
    ...
    <template v-if="!unmapped">
      <div class="w-full mx-[24px]" v-if="step === 7 || step === 8">
        <h1
          class="text-[24px] mb-[36px] text-t_black-800 dark:text-t_white-200 font-fira font-semibold"
          v-if="step === 8"
        >
          Edit scan
        </h1>
      ...
      <div class="w-full mx-[24px]" v-else>
        <h1
          class="text-[18px] text-t_black-800 dark:text-t_white-200 italic font-fira"
          v-if="step === 5 || step === 6"
        >
          {{ form.words }}
        </h1>
